  ...
  async writeRiskControlMapping(
    ...
    if (this.supportsLiveQueries() && (justification || gaps || recommendations)) {
      const narrativeContent = [
        '🔍 AI RISK-CONTROL MAPPING ANALYSIS',
        '',
        'RATIONALE:',
        justification || '',
        '',
        ...
        recommendations || '',
        '',
        `Generated by Ema Risk-Control Mapping Agent · ${new Date().toISOString()}`
      ].join('\n');

      await this.restUpdate('grc__Risk__c', riskSysId, {
        ai_recommendation__c: narrativeContent.substring(0, 32768)
      });
      console.log(`[DynamicAdapter:${this.config.platformName}] Updated grc__Risk__c ${riskSysId}.ai_recommendation__c with mapping narrative`);
    }
    ...
    if (this.supportsLiveQueries()) {
      try {
        ...
        const emaPayload: any = {
          ...
          Ema_Audit_Summary__c: (payload.html || payload.summary || '').substring(0, 32768),
